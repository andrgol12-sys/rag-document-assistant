...
# Тестовые вопросы по текущей базе знаний (транспорт / температура)
EVALUATION_QUESTIONS = [
    "Какая температура нужна для перевозки замороженной продукции?",
    "Что водитель обязан проверить перед началом перевозки?",
    "В каких регионах работает компания?",
    "Что делать при отклонении температуры во время перевозки?",
    "Какие обязанности у логиста перед подачей автомобиля?",
]
...
def prepare_dataset(pipeline: RAGPipeline, questions: list) -> Dataset:
    ...
    for i, question in enumerate(questions, 1):
        ...
        # Контекст — список текстов из найденных документов (dict с полем text)
        ...
        if result.get("llm_skipped"):
            print("     [!] LLM не вызывался (нет релевантного контекста)")
        else:
