...
class RAGPipeline:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            data_dir: путь к папке с .txt документами
        ...
        # Загрузка документов из папки (уже загруженные файлы пропускаются)
        print(f"Загрузка документов из {data_dir}...")
        self.vector_store.load_documents(data_dir)
...
Ответ:"""
    ...
    def query(self, user_query: str, use_cache: bool = True) -> Dict[str, Any]:
        ...
        retrieval_stats = getattr(self.vector_store, "last_retrieval_stats", None)
        print(f"[+] Найдено {len(context_docs)} релевантных документов")

        # Нет релевантного контекста — не вызываем LLM впустую
        if not context_docs:
            answer = (
                "В базе знаний нет информации, достаточной для ответа на данный вопрос."
            )
            return {
                "query": user_query,
                "answer": answer,
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Тестовые запросы по базе знаний
        test_queries = [
            "Какая температура нужна для перевозки замороженной продукции?",
